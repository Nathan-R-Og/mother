...
class EbbPresent(object):
    def __init__(self, lines) -> None:
        ob_header = lines[0].split(",")
        self.obtype = 0x20
        self.x = int(ob_header[1].strip())
        self.direction = ob_header[2].strip()
        self.y = int(ob_header[3].strip())
        self.item = int(lines[2].split(" ")[1])
        self.flag = int(lines[3].split(" ")[1])
        print(f"{self.x}, {self.y}, {self.item}, {self.flag}")
...
for file in files:
    ...
    while i < len(lines):
        ...
        if line.startswith("objectDef OBJ_TYPE::PRESENT"):
            presents.append(EbbPresent(lines[i:i+4]))
            i += 4-1
        i += 1

mus_epiloguejp_pulse2_phrase03:
    Set_Timbre 5, 19, timbre_c_square SQTD_50, 1, SQTV_ISVOL, 1
    Note_Length nl_whole
    Note_G 5
    Note_Length nl_half
    Note_C 6
    Note_B 5

    Note_Length nl_whole
    Note_G 5
    Note_Length nl_half
    Note_D 5
    Note_C 5

    Note_Length nl_whole
    Note_C 5
    Note_Length nl_half
    Note_AS 4
    Note_A 4

    Note_Length nl_whole
    Note_A 4
    Note_D 5

    Track_End

mus_epiloguejp_pulse1_phrase03:
    Set_Timbre 0, 14, timbre_c_square SQTD_50, 1, SQTV_ISVOL, 1
    Note_Length nl_half
    Note_C 4
    Note_G 4
    Note_E 5
    Note_D 5

    Note_B 3
    Note_G 4
    Note_Length nl_whole
    Note_B 4

    Note_Length nl_half
    Note_AS 3
    Note_G 4
    Note_Length nl_whole
    Note_D 5

    Note_Length nl_half
    Note_A 3
    Note_D 4
    Note_Length nl_whole
    Note_FS 4

    Track_End

mus_epiloguejp_triangle_phrase03:
    Note_Repeat 8
        Note_Length nl_whole
        Note_Rest
    Note_End_Repeat

    Track_End

mus_epiloguejp_noise_phrase03:
    Note_Repeat 8
        Note_Length nl_whole
        Note_Noise 0, 1
    Note_End_Repeat

    Track_End

mus_epiloguejp_pulse1_phrase07:
    Set_Transpose 1, 13
    Track_End

mus_epiloguejp_pulse2_phrase01:

    Set_Timbre 0, 16, timbre_c_square SQTD_50, 1, SQTV_ISVOL, 5
    Note_Length nl_quarter
    Note_D 6
    Note_E 6
    Note_FS 6
    Note_A 6
    Note_Length nl_whole
    Note_E 6

    Note_Length nl_quarter
    Note_D 7
    Note_CS 7
    Note_B 6
    Note_FS 6
    Note_Length nl_whole
    Note_A 6

    Note_Length nl_quarter
    Note_B 6
    Note_CS 7
    Note_D 7
    Note_Length nl_half
    Note_A 6
    Note_Length nl_d_half
    Note_D 6

    Note_Length nl_quarter
    Note_G 6
    Note_FS 6
    Note_D 6
    Note_Length nl_whole
    Note_A 5
    Note_Length nl_quarter
    Note_Rest

    Note_Length nl_half
    Note_B 5
    Note_CS 6
    Note_D 6
    Note_G 6

    Note_Length nl_quarter
    Note_FS 6
    Note_G 6
    Note_A 6
    Note_FS 6
    Note_E 6
    Note_FS 6
    Note_G 6
    Note_E 6

    Note_Length nl_quarter
    Note_B 5
    Note_G 6
    Note_FS 6
    Note_A 5
    Note_Length nl_half
    Note_E 6
    Note_CS 6

    Note_Length nl_half
    Note_D 6
    Note_Length nl_quarter
    Note_A 5
    Note_E 6
    Note_Length nl_whole
    Note_D 6

    Track_End

mus_epiloguejp_pulse1_phrase01:
    Set_Timbre 0, 12, timbre_c_square SQTD_50, 1, SQTV_ISVOL, 1
    Note_Length nl_quarter
    Note_D 5
    Note_FS 5
    Note_A 5
    Note_D 6
    Note_G 4
    Note_D 5
    Note_G 5
    Note_B 5

    Note_B 4
    Note_D 5
    Note_FS 5
    Note_B 5
    Note_FS 4
    Note_CS 5
    Note_FS 5
    Note_A 5

    Note_G 4
    Note_D 5
    Note_G 5
    Note_FS 5
    Note_Rest
    Note_FS 5
    Note_A 5
    Note_D 6

    Note_G 4
    Note_D 5
    Note_G 5
    Note_CS 5
    Note_Rest
    Note_E 5
    Note_A 5
    Note_CS 6

    Note_G 4
    Note_G 5
    Note_A 4
    Note_A 5
    Note_B 4
    Note_B 5
    Note_CS 5
    Note_CS 6

    Note_D 5
    Note_FS 5
    Note_A 5
    Note_FS 5
    Note_A 4
    Note_CS 5
    Note_E 5
    Note_A 5

    Note_G 4
    Note_D 5
    Note_G 5
    Note_D 5
    Note_A 4
    Note_E 5
    Note_A 5
    Note_CS 6

    Note_D 5
    Note_A 5
    Note_Rest
    Note_Rest
    Note_FS 6
    Note_D 6
    Note_A 5
    Note_FS 5

    Track_End

mus_epiloguejp_triangle_phrase01:
    Note_Repeat 16
        Note_Length nl_whole
        Note_Rest
    Note_End_Repeat

    Track_End

mus_epiloguejp_noise_phrase01:
    Note_Repeat 16
        Note_Length nl_whole
        Note_Noise 0, 1
    Note_End_Repeat

    Track_End

mus_epiloguejp_pulse2_phrase04:
    Set_Timbre 5, 7, timbre_c_square SQTD_75, 1, SQTV_ISVOL, 6

    Note_Repeat 3
        Note_Length nl_half
        Note_B 4
        Note_Length nl_quarter
        Note_C 5
        Note_D 5
        Note_Length nl_half
        Note_D 5
        Note_E 5

        Note_Length nl_half
        Note_A 4
        Note_Length nl_quarter
        Note_B 4
        Note_C 5
        Note_Length nl_half
        Note_B 4
        Note_G 4

        Note_Length nl_half
        Note_G 4
        Note_Length nl_quarter
        Note_A 4
        Note_E 4
        Note_Length nl_quarter
        Note_FS 4
        Note_G 4
        Note_A 4
        Note_B 4

        Note_Length nl_half
        Note_FS 4
        Note_E 4
        Note_Length nl_whole
        Note_D 4
    Note_End_Repeat

    Note_Length nl_half
    Note_B 4
    Note_Length nl_quarter
    Note_D 5
    Note_G 5
    Note_Length nl_half
    Note_E 5
    Note_C 5

    Note_Length nl_half
    Note_D 5
    Note_Length nl_quarter
    Note_A 4
    Note_C 5
    Note_Length nl_d_half
    Note_B 4
    Note_Length nl_eighth
    Note_B 4
    Note_A 4

    Note_Length nl_half
    Note_G 4
    Note_Length nl_quarter
    Note_A 4
    Note_B 4
    Note_Length nl_half
    Note_A 4
    Note_E 4

    Note_G 4
    Note_FS 4
    Note_G 4
    Note_A 4

    Set_Timbre 5, 7, timbre_c_square SQTD_75, 1, SQTV_ISVOL, 3
    Note_Length nl_whole
    Note_B 4
    Note_Length nl_half
    Note_A 4
    Note_D 5

    Note_Length nl_whole
    Note_B 4
    Note_A 4

    Set_Timbre 0, 9, timbre_c_square SQTD_75, 1, SQTV_ISVOL, 2
    Note_Length nl_whole
    Note_A 4

    Track_End

mus_epiloguejp_pulse1_phrase04:
    Set_Timbre 5, 7, timbre_c_square SQTD_50, 1, SQTV_ISVOL, 2

    Note_Repeat 2
        Note_Length nl_eighth
        Note_G 5
        Note_D 5
        Note_B 4
        Note_G 4
        Note_D 4
        Note_G 4
        Note_B 4
        Note_D 5
        Note_G 5
        Note_E 5
        Note_C 5
        Note_G 4
        Note_E 4
        Note_G 4
        Note_C 5
        Note_E 5

        Note_FS 5
        Note_D 5
        Note_A 4
        Note_FS 4
        Note_D 4
        Note_FS 4
        Note_A 4
        Note_D 5
        Note_D 5
        Note_B 4
        Note_G 4
        Note_D 4
        Note_B 3
        Note_D 4
        Note_G 4
        Note_B 4

        Note_D 5
        Note_B 4
        Note_G 4
        Note_E 4
        Note_CS 4
        Note_E 4
        Note_G 4
        Note_B 4
        Note_CS 5
        Note_A 4
        Note_E 4
        Note_CS 4
        Note_A 3
        Note_CS 4
        Note_E 4
        Note_A 4

        Note_D 5
        Note_A 4
        Note_FS 4
        Note_D 4
        Note_A 3
        Note_D 4
        Note_FS 4
        Note_A 4
        Note_D 5
        Note_C 5
        Note_A 4
        Note_FS 4
        Note_D 4
        Note_C 4
        Note_A 3
        Note_FS 3
    Note_End_Repeat

;fallthrough
mus_epiloguejp_pulse1_phrase05:
    Note_Repeat 2
        Note_Length nl_sixteenth
        Note_G 5
        Note_D 5
        Note_B 4
        Note_G 4
        Note_D 4
        Note_G 4
        Note_B 4
        Note_D 5
    Note_End_Repeat
    Note_Repeat 2
        Note_G 5
        Note_E 5
        Note_C 5
        Note_G 4
        Note_E 4
        Note_G 4
        Note_C 5
        Note_E 5
    Note_End_Repeat

    Note_Repeat 2
        Note_FS 5
        Note_D 5
        Note_A 4
        Note_FS 4
        Note_D 4
        Note_FS 4
        Note_A 4
        Note_D 5
    Note_End_Repeat
    Note_Repeat 2
        Note_D 5
        Note_B 4
        Note_G 4
        Note_D 4
        Note_B 3
        Note_D 4
        Note_G 4
        Note_B 4
    Note_End_Repeat

    Note_D 5
    Note_B 4
    Note_G 4
    Note_E 4
    Note_D 4
    Note_B 3
    Note_G 3
    Note_E 3
    Note_D 3
    Note_E 3
    Note_G 3
    Note_B 3
    Note_E 4
    Note_G 4
    Note_B 4
    Note_D 5
    Note_E 5
    Note_CS 5
    Note_A 4
    Note_G 4
    Note_E 4
    Note_CS 4
    Note_A 3
    Note_G 3
    Note_E 3
    Note_A 3
    Note_CS 4
    Note_E 4
    Note_G 4
    Note_A 4
    Note_CS 5
    Note_E 5

    Note_Repeat 2
        Note_D 5
        Note_A 4
        Note_FS 4
        Note_D 4
        Note_A 3
        Note_D 4
        Note_FS 4
        Note_A 4
    Note_End_Repeat
    Note_Repeat 2
        Note_D 5
        Note_C 5
        Note_A 4
        Note_FS 4
        Note_D 4
        Note_C 4
        Note_A 3
        Note_FS 3
    Note_End_Repeat
    Track_End

mus_epiloguejp_pulse1_phrase06:
    Set_Timbre 5, 9, timbre_c_square SQTD_50, 1, SQTV_ISVOL, 2

    Note_Repeat 2
        Note_Length nl_quarter
        Note_G 5
        Note_D 5
        Note_B 4
        Note_D 5
        Note_G 5
        Note_C 5
        Note_A 4
        Note_C 5
    Note_End_Repeat

    Set_Timbre 0, 14, timbre_c_square SQTD_50, 1, SQTV_ISVOL, 1
    Note_G 5
    Note_C 5
    Note_A 4
    Note_C 5
    Track_End

mus_epiloguejp_triangle_phrase04:
    Note_Repeat 2
        Note_Length nl_whole
        Note_G 4
        Note_C 4
        Note_D 4
        Note_G 4
        Note_E 4
        Note_A 4
        Note_D 4
        Note_Length nl_half
        Note_D 4
        Note_C 4
    Note_End_Repeat

    Note_Length nl_half
    Note_G 3
    Note_Length nl_quarter
    Note_A 3
    Note_B 3
    Note_Length nl_half
    Note_C 4
    Note_C 3
    Note_Length nl_half
    Note_D 3
    Note_Length nl_quarter
    Note_FS 3
    Note_A 3
    Note_Length nl_d_half
    Note_G 3
    Note_Length nl_quarter
    Note_FS 3
    Note_E 3
    Note_D 3
    Note_Length nl_half
    Note_CS 3
    Note_Length nl_whole
    Note_A 3
    Note_D 3
    Note_Length nl_half
    Note_D 4
    Note_C 4
    Note_Length nl_half
    Note_G 4
    Note_Length nl_quarter
    Note_A 4
    Note_B 4
    Note_Length nl_half
    Note_C 5
    Note_C 4
    Note_Length nl_half
    Note_D 4
    Note_Length nl_quarter
    Note_FS 4
    Note_A 4
    Note_Length nl_d_half
    Note_G 4
    Note_Length nl_quarter
    Note_FS 4
    Note_E 4
    Note_D 4
    Note_Length nl_half
    Note_CS 4
    Note_Length nl_whole
    Note_A 3
    Note_D 3
    Note_B 3
    Note_G 3
    Note_F 3
    Note_G 3
    Note_F 3
    Note_F 3
    Track_End

mus_epiloguejp_noise_phrase04:
    Note_Repeat 7
        Note_Length nl_half
        Note_Noise 1, 1
        Note_Noise 0, 4
        Note_Length nl_d_quarter
        Note_Noise 1, 1
        Note_Length nl_eighth
        Note_Noise 0, 4
        Note_Length nl_half
        Note_Noise 0, 7
    Note_End_Repeat

    Note_Length nl_half
    Note_Noise 1, 1
    Note_Length nl_d_quarter
    Note_Noise 2, 1
    Note_Length nl_sixteenth
    Note_Noise 1, 1
    Note_Noise 1, 1
    Note_Length nl_eighth
    Note_Noise 2, 1
    Note_Noise 1, 1
    Note_Noise 0, 1
    Note_Noise 1, 1
    Note_Noise 0, 1
    Note_Noise 1, 1
    Note_Noise 2, 1
    Note_Noise 0, 1

    Note_Repeat 8
        Note_Length nl_quarter
        Note_Noise 1, 4
        Note_Noise 0, 4
        Note_Noise 2, 1
        Note_Noise 0, 4
        Note_Length nl_eighth
        Note_Noise 0, 4
        Note_Noise 1, 1
        Note_Noise 1, 4
        Note_Noise 0, 1
        Note_Length nl_quarter
        Note_Noise 2, 1
        Note_Noise 0, 4
    Note_End_Repeat

    Note_Repeat 4
        Note_Length nl_half
        Note_Noise 0, 1
        Note_Noise 0, 4
    Note_End_Repeat

    Track_End


;why is this out of order ????????????????????????
mus_epiloguejp_pulse2_start:
.addr mus_epiloguejp_pulse2_phrase01
.addr mus_epiloguejp_pulse2_phrase02
.addr mus_epiloguejp_pulse2_phrase03
.addr mus_epiloguejp_pulse2_phrase04
mus_epiloguejp_pulse2_loop:
.addr mus_epiloguejp_pulse2_phrase01
.addr -1
.addr mus_epiloguejp_pulse2_loop



mus_epiloguejp_pulse1_start:
.addr mus_epiloguejp_pulse1_phrase01
.addr mus_epiloguejp_pulse1_phrase02
.addr mus_epiloguejp_pulse1_phrase03
.addr mus_epiloguejp_pulse1_phrase04
.addr mus_epiloguejp_pulse1_phrase05
.addr mus_epiloguejp_pulse1_phrase06
.addr mus_epiloguejp_pulse1_phrase07
mus_epiloguejp_pulse1_loop:
.addr mus_epiloguejp_pulse1_phrase01
.addr -1
.addr mus_epiloguejp_pulse1_loop



mus_epiloguejp_triangle_start:
.addr mus_epiloguejp_triangle_phrase01
.addr mus_epiloguejp_triangle_phrase02
.addr mus_epiloguejp_triangle_phrase03
.addr mus_epiloguejp_triangle_phrase04
mus_epiloguejp_triangle_loop:
.addr mus_epiloguejp_triangle_phrase01
.addr -1
.addr mus_epiloguejp_triangle_loop



mus_epiloguejp_noise_start:
.addr mus_epiloguejp_noise_phrase01
.addr mus_epiloguejp_noise_phrase02
.addr mus_epiloguejp_noise_phrase03
.addr mus_epiloguejp_noise_phrase04
mus_epiloguejp_noise_loop:
.addr mus_epiloguejp_noise_phrase01
.addr -1
.addr mus_epiloguejp_noise_loop



mus_epiloguejp_pulse2_phrase02:
    Set_Timbre 0, 16, timbre_c_square SQTD_50, 1, SQTV_ISVOL, 1

    Note_Repeat 2
        Note_Length nl_quarter
        Note_D 5
        Note_E 5
        Note_FS 5
        Note_A 5
        Note_Length nl_whole
        Note_E 5

        Note_Length nl_quarter
        Note_D 6
        Note_CS 6
        Note_B 5
        Note_FS 5
        Note_Length nl_whole
        Note_A 5

        Note_Length nl_quarter
        Note_B 5
        Note_CS 6
        Note_D 6
        Note_Length nl_half
        Note_A 5
        Note_Length nl_d_half
        Note_D 5

        Note_Length nl_quarter
        Note_G 5
        Note_FS 5
        Note_D 5
        Note_Length nl_whole
        Note_A 4
        Note_Length nl_quarter
        Note_Rest

        Note_Length nl_half
        Note_B 4
        Note_CS 5
        Note_D 5
        Note_G 5

        Note_Length nl_quarter
        Note_FS 5
        Note_G 5
        Note_A 5
        Note_FS 5
        Note_E 5
        Note_FS 5
        Note_G 5
        Note_E 5

        Note_Length nl_quarter
        Note_B 4
        Note_G 5
        Note_FS 5
        Note_A 4
        Note_Length nl_half
        Note_E 5
        Note_CS 5

        Note_Length nl_half
        Note_D 5
        Note_Length nl_quarter
        Note_A 4
        Note_E 5
        Note_Length nl_whole
        Note_D 5
    Note_End_Repeat

    Track_End

mus_epiloguejp_pulse1_phrase02:
    Set_Timbre 5, 20, timbre_c_square SQTD_50, 1, SQTV_ISVOL, 1
    Note_Length nl_quarter
    Note_A 5
    Note_FS 6
    Note_A 5
    Note_FS 6
    Note_B 5
    Note_G 6
    Note_B 5
    Note_G 6

    Note_B 5
    Note_FS 6
    Note_B 5
    Note_FS 6
    Note_A 5
    Note_FS 6
    Note_A 5
    Note_FS 6

    Note_B 5
    Note_G 6
    Note_B 5
    Note_Length nl_half
    Note_A 5
    Note_Length nl_quarter
    Note_FS 6
    Note_A 5
    Note_FS 6

    Note_B 5
    Note_G 6
    Note_B 5
    Note_Length nl_half
    Note_CS 6
    Note_Length nl_quarter
    Note_A 6
    Note_CS 6
    Note_A 6

    Note_B 5
    Note_G 6
    Note_B 5
    Note_Length nl_half
    Note_G 6
    Note_Length nl_quarter
    Note_B 5
    Note_Length nl_half
    Note_G 6

    Note_Length nl_quarter
    Note_A 5
    Note_FS 6
    Note_A 5
    Note_FS 6
    Note_G 5
    Note_E 6
    Note_G 5
    Note_E 6

    Note_B 5
    Note_G 6
    Note_B 5
    Note_G 6
    Note_CS 6
    Note_A 6
    Note_CS 6
    Note_A 6

    Note_D 6
    Note_FS 6
    Note_A 6
    Note_FS 6
    Note_D 7
    Note_A 6
    Note_FS 6
    Note_D 6

    Set_Timbre 5, 9, timbre_c_square SQTD_50, 1, SQTV_ISVOL, 2
    Note_Length nl_eighth
    Note_A 4
    Note_FS 4
    Note_A 4
    Note_FS 4
    Note_B 4
    Note_FS 4
    Note_A 4
    Note_FS 4
    Note_A 4
    Note_G 4
    Note_A 4
    Note_G 4
    Note_B 4
    Note_G 4
    Note_A 4
    Note_G 4

    Note_A 4
    Note_FS 4
    Note_A 4
    Note_FS 4
    Note_B 4
    Note_FS 4
    Note_A 4
    Note_FS 4
    Note_A 4
    Note_FS 4
    Note_A 4
    Note_FS 4
    Note_B 4
    Note_FS 4
    Note_A 4
    Note_FS 4

    Note_A 4
    Note_G 4
    Note_A 4
    Note_G 4
    Note_B 4
    Note_G 4
    Note_Length nl_d_quarter
    Note_A 4
    Note_Length nl_eighth
    Note_FS 4
    Note_A 4
    Note_FS 4
    Note_B 4
    Note_FS 4
    Note_Length nl_d_quarter
    Note_A 4

    Note_Length nl_eighth
    Note_G 4
    Note_A 4
    Note_G 4
    Note_B 4
    Note_G 4
    Note_Length nl_d_quarter
    Note_A 4
    Note_Length nl_eighth
    Note_E 4
    Note_A 4
    Note_E 4
    Note_B 4
    Note_E 4
    Note_Length nl_quarter
    Note_A 4

    Note_Length nl_eighth
    Note_B 4
    Note_G 4
    Note_B 4
    Note_G 4
    Note_CS 5
    Note_G 4
    Note_CS 5
    Note_G 4
    Note_D 5
    Note_G 4
    Note_D 5
    Note_G 4
    Note_E 5
    Note_G 4
    Note_E 5
    Note_G 4

    Note_FS 5
    Note_D 5
    Note_A 4
    Note_FS 4
    Note_D 5
    Note_A 4
    Note_FS 4
    Note_D 4
    Note_E 4
    Note_A 4
    Note_CS 5
    Note_A 4
    Note_E 5
    Note_CS 5
    Note_A 4
    Note_CS 5

    Note_B 4
    Note_D 4
    Note_B 4
    Note_D 4
    Note_B 4
    Note_D 4
    Note_B 4
    Note_D 4
    Note_CS 5
    Note_E 4
    Note_CS 5
    Note_E 4
    Note_CS 5
    Note_E 4
    Note_CS 5
    Note_E 4

    Note_D 5
    Note_FS 4
    Note_D 5
    Note_FS 4
    Note_D 5
    Note_FS 4
    Note_D 5
    Note_FS 4
    Note_D 5
    Note_A 4
    Note_FS 4
    Note_D 4
    Note_A 4
    Note_FS 4
    Note_D 4
    Note_FS 4

    Track_End

mus_epiloguejp_triangle_phrase02:
    Set_Timbre 5, 0, timbre_c_triangle 0, 0

    Note_Repeat 2
        Note_Length nl_whole
        Note_D 4
        Note_G 4

        Note_B 3
        Note_FS 3

        Note_Length nl_d_half
        Note_G 3
        Note_Length nl_whole
        Note_D 4
        Note_Length nl_quarter
        Note_Rest

        Note_Length nl_d_half
        Note_G 3
        Note_Length nl_whole
        Note_A 3
        Note_Length nl_quarter
        Note_Rest

        Note_Length nl_half
        Note_G 3
        Note_A 3
        Note_B 3
        Note_CS 4

        Note_Length nl_whole
        Note_D 4
        Note_A 3

        Note_Length nl_whole
        Note_G 3
        Note_A 3

        Note_D 4
        Note_D 3
    Note_End_Repeat

    Track_End

mus_epiloguejp_noise_phrase02:
    Note_Repeat 16
        Note_Length nl_whole
        Note_Noise 0, 1
    Note_End_Repeat

    Note_Repeat 16
        Note_Length nl_half
        Note_Noise 0, 1
        Note_Noise 0, 4
    Note_End_Repeat

    Track_End
